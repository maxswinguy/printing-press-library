...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "amazon-orders-pp-cli",
		Short: `Sync your Amazon order history into a local SQLite store, then ask cross-cutting questions offline: which deliveries are slipping, what did I spend last quarter, when did I order that thing, where is…`,
		Long: `Sync your Amazon order history into a local SQLite store, then ask cross-cutting questions offline: which deliveries are slipping, what did I spend last quarter, when did I order that thing, where is…
...
Health check: run 'amazon-orders-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("amazon-orders-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'amazon-orders-pp-cli profile list')")
	...
	rootCmd.AddCommand(newGiftCardsPromotedCmd(flags))
	rootCmd.AddCommand(newShipmentsPromotedCmd(flags))
	rootCmd.AddCommand(newTransactionsPromotedCmd(flags))

	// Novel transcendence commands (hand-written, beyond the spec).
	rootCmd.AddCommand(newWhereIsMyStuffCmd(flags))
	rootCmd.AddCommand(newArrivingSoonCmd(flags))
	rootCmd.AddCommand(newLateCmd(flags))
	rootCmd.AddCommand(newFindCmd(flags))
	rootCmd.AddCommand(newSpendCmd(flags))
	rootCmd.AddCommand(newTopItemsCmd(flags))
	rootCmd.AddCommand(newTrackCmd(flags))
	rootCmd.AddCommand(newDeliverySlipsCmd(flags))
	rootCmd.AddCommand(newSubscribeAndSaveCmd(flags))
	rootCmd.AddCommand(newReturnsCmd(flags))
	rootCmd.AddCommand(newCarriersCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("amazon-orders-pp-cli %s\n", version)
		},
	}
}
