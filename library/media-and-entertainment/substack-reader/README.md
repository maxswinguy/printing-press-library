...
Substack Reader is built to read only what you are entitled to, on your own behalf:

- **Your own credential, never shared.** Tier-1 access uses *your* `substack.sid` session cookie to unlock the paid posts *you already subscribe to* — functionally identical to your own browser making the same request. The cookie is sent only to Substack, never logged, and never redistributed.
- **Opt-in, never required.** Free/public posts work fully keyless; a session is only ever needed for paid content you already pay for.
- **Read-only.** This CLI never creates, updates, deletes, publishes, or otherwise mutates anything on Substack.
- **Not a bulk crawler.** Do not use it to bulk-scrape or redistribute paid content you are not entitled to; it reads on demand and self-throttles.
...
1. Per-kind env var: `SUBSTACK_CONFIG_DIR`, `SUBSTACK_DATA_DIR`, `SUBSTACK_STATE_DIR`, or `SUBSTACK_CACHE_DIR`
2. `--home <dir>` for this invocation
3. `SUBSTACK_HOME` for a flat relocated root
...
```bash
export SUBSTACK_HOME=/srv/substack
substack-reader-pp-cli doctor
```

Under `SUBSTACK_HOME=/srv/substack`, the four dirs resolve to `/srv/substack/config`, `/srv/substack/data`, `/srv/substack/state`, and `/srv/substack/cache`.
...
Precedence matters in fleets: an ambient per-kind variable such as `SUBSTACK_DATA_DIR` overrides an explicit `--home` for that kind. Use `SUBSTACK_HOME` or the per-kind variables for durable fleet relocation; treat `--home` as the weaker per-invocation lever.

Relocation is one-way. Unsetting `SUBSTACK_HOME` does not move files back to platform defaults, and `doctor` cannot find files left under a former root. Move the files manually before unsetting relocation variables.
...
Run `substack-reader-pp-cli doctor` to see the resolved config, data, state, and cache directories. The platform-default config path is `~/.config/substack-reader-pp-cli/config.toml`; `--home`, `SUBSTACK_HOME`, and per-kind env vars can relocate it.
