# Substack Printed CLI Agent Guide

This directory is a generated `substack-reader-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
substack-reader-pp-cli doctor --json
substack-reader-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
substack-reader-pp-cli which "<capability>" --json
substack-reader-pp-cli <command> --help
```
...
```bash
substack-reader-pp-cli <command> --agent
```
...
```bash
substack-reader-pp-cli <command> --help
substack-reader-pp-cli <command> --dry-run --agent
```
