...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  substack-reader-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  substack-reader-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  substack-reader-pp-cli workflow status

  # Show status as JSON
  substack-reader-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("substack-reader-pp-cli")
			}
