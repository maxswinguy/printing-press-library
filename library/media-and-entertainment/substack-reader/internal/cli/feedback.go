...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SUBSTACK_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SUBSTACK_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "substack-reader-pp-cli/feedback")
...
When ` + "`SUBSTACK_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SUBSTACK_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  substack-reader-pp-cli feedback list
  substack-reader-pp-cli feedback list --limit 5
  substack-reader-pp-cli feedback list --json`,
