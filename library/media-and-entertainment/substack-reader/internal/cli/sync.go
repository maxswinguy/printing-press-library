...
Resource scoping:
  ...
  substack-reader-pp-cli sync

  # Sync specific resources only
  substack-reader-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  substack-reader-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  substack-reader-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  substack-reader-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  substack-reader-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "publications":
		return db.UpsertPublications(data)
	default:
