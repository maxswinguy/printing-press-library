...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "suno-pp-cli",
		Short: `Suno CLI — The correct, offline-first Suno CLI — every feature the abandoned clients have, plus a local SQLite library, full-text …`,
		Long: `Suno CLI — The correct, offline-first Suno CLI — every feature the abandoned clients have, plus a local SQLite library, full-text …
...
Health check: run 'suno-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("suno-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'suno-pp-cli profile list')")
	...
	rootCmd.AddCommand(newClipsCmd(flags))
	...
	rootCmd.AddCommand(newBillingPromotedCmd(flags))
	rootCmd.AddCommand(newPersonaCmd(flags))
	...
	rootCmd.AddCommand(newSkillCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())

	// Generation/transform commands are grouped under the (hidden) `generate`
	// parent: create/describe/extend/cover/remaster plus the
	// concat/lyrics/lyrics-status/video-status endpoint subcommands.
	rootCmd.AddCommand(newGenerateCmd(flags))

	// Hand-built user-facing download/status commands.
	rootCmd.AddCommand(newSunoStatusCmd(flags))
	rootCmd.AddCommand(newSunoDownloadCmd(flags))
	rootCmd.AddCommand(newSunoModelsCmd(flags))

	// Novel "transcendence" commands — local-store analytics over synced clips.
	rootCmd.AddCommand(newSunoGrepCmd(flags))
	rootCmd.AddCommand(newSunoAnalyticsCmd(flags))
	rootCmd.AddCommand(newSunoLineageCmd(flags))
	rootCmd.AddCommand(newSunoTopCmd(flags))
	rootCmd.AddCommand(newSunoSQLCmd(flags))
	rootCmd.AddCommand(newSunoCreditsCmd(flags))

	// Restored from the 2026-05-15 build (backward compatibility) — coexist
	// with the novel commands above; no aliasing.
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "version",
		Short: "Print the installed suno-pp-cli version string",
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("suno-pp-cli %s\n", version)
		},
	}
}
