...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "dice-fm-pp-cli",
		Short: `Every ticket, fan, and pound of revenue from your DICE events — queryable, exportable, and joinable across shows.`,
		Long: `Every ticket, fan, and pound of revenue from your DICE events — queryable, exportable, and joinable across shows.
...
Health check: run 'dice-fm-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("dice-fm-pp-cli {{ .Version }}\n")
	...
	rootCmd.AddCommand(newExtrasPromotedCmd(flags))
	rootCmd.AddCommand(newGenresPromotedCmd(flags))
	rootCmd.AddCommand(newOrdersPromotedCmd(flags))
	// `returns` is a leaf list command; attach the novel `anomalies`
	// subcommand to it so `returns anomalies` works while `returns` still
	// lists returns.
	returnsCmd := newReturnsPromotedCmd(flags)
	returnsCmd.AddCommand(newReturnsAnomaliesCmd(flags))
	rootCmd.AddCommand(returnsCmd)
	rootCmd.AddCommand(newTicketsPromotedCmd(flags))
	rootCmd.AddCommand(newTransfersPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())

	// Novel transcendence commands (analytics over the live API and local store).
	rootCmd.AddCommand(newDoorCmd(flags))
