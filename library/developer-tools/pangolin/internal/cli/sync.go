...
Resource scoping:
  ...
  pangolin-pp-cli sync

  # Sync specific resources only
  pangolin-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  pangolin-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  pangolin-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  pangolin-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  pangolin-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("pangolin-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/pangolin-pp-cli/data.db)")
...
func extractPageItems(data json.RawMessage, cursorParam string) ([]json.RawMessage, string, bool) {
	...
	// failed and the top-level array-count fallback found no peers, peek
	// one level into a "data" sub-envelope and re-run the same scan.
	for _, dataKey := range []string{"data", "Data", "result", "Result"} {
		...
		for _, key := range pageItemKeys {
			if innerRaw, ok := inner[key]; ok {
				if err := json.Unmarshal(innerRaw, &items); err == nil && len(items) > 0 {
					nextCursor, hasMore := extractPaginationFromEnvelope(inner, cursorParam)
					return items, nextCursor, hasMore
				}
			}
		}
		var innerArrayItems []json.RawMessage
		innerArrayCount := 0
		for _, innerRaw := range inner {
			var candidate []json.RawMessage
			if err := json.Unmarshal(innerRaw, &candidate); err == nil && len(candidate) > 0 {
				innerArrayItems = candidate
				innerArrayCount++
			}
		}
		if innerArrayCount == 1 {
			nextCursor, hasMore := extractPaginationFromEnvelope(inner, cursorParam)
			return innerArrayItems, nextCursor, hasMore
		}
	}
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "client_site_resources":
		return db.UpsertClientSiteResources(data)
	case "unarchive":
		return db.UpsertUnarchive(data)
	case "unblock":
		return db.UpsertUnblock(data)
	case "oidc":
		return db.UpsertOidc(data)
	case "idp_org":
		return db.UpsertIdpOrg(data)
	case "org_access_tokens":
		return db.UpsertOrgAccessTokens(data)
	case "alert_rule":
		return db.UpsertAlertRule(data)
	case "alert_rules":
		return db.UpsertAlertRules(data)
	case "api_key":
		return db.UpsertApiKey(data)
	case "api_keys":
		return db.UpsertApiKeys(data)
	case "blueprint":
		return db.UpsertBlueprint(data)
	case "blueprints":
		return db.UpsertBlueprints(data)
	case "org_certificate":
		return db.UpsertOrgCertificate(data)
	case "org_client":
		return db.UpsertOrgClient(data)
	case "org_clients":
		return db.UpsertOrgClients(data)
	case "create_invite":
		return db.UpsertCreateInvite(data)
	case "org_domain":
		return db.UpsertOrgDomain(data)
	case "org_domains":
		return db.UpsertOrgDomains(data)
	case "event_streaming_destination":
		return db.UpsertEventStreamingDestination(data)
	case "health_check":
		return db.UpsertHealthCheck(data)
	case "health_checks":
		return db.UpsertHealthChecks(data)
	case "org_idp":
		return db.UpsertOrgIdp(data)
	case "invitations":
		return db.UpsertInvitations(data)
	case "logs":
		return db.UpsertLogs(data)
	case "pick_client_defaults":
		return db.UpsertPickClientDefaults(data)
	case "pick_site_defaults":
		return db.UpsertPickSiteDefaults(data)
	case "reset_bandwidth":
		return db.UpsertResetBandwidth(data)
	case "org_resource":
		return db.UpsertOrgResource(data)
	case "resources":
		return db.UpsertResources(data)
	case "resources_names":
		return db.UpsertResourcesNames(data)
	case "org_role":
		return db.UpsertOrgRole(data)
	case "org_roles":
		return db.UpsertOrgRoles(data)
	case "org_site":
		return db.UpsertOrgSite(data)
	case "org_site_resource":
		return db.UpsertOrgSiteResource(data)
	case "org_site_resources":
		return db.UpsertOrgSiteResources(data)
	case "sites":
		return db.UpsertSites(data)
	case "org_user":
		return db.UpsertOrgUser(data)
	case "user_by_username":
		return db.UpsertUserByUsername(data)
	case "user_devices":
		return db.UpsertUserDevices(data)
	case "org_users":
		return db.UpsertOrgUsers(data)
	case "resource_access_token":
		return db.UpsertResourceAccessToken(data)
	case "resource_access_tokens":
		return db.UpsertResourceAccessTokens(data)
	case "header_auth":
		return db.UpsertHeaderAuth(data)
	case "password":
		return db.UpsertPassword(data)
	case "pincode":
		return db.UpsertPincode(data)
	case "resource_roles":
		return db.UpsertResourceRoles(data)
	case "rule":
		return db.UpsertRule(data)
	case "rules":
		return db.UpsertRules(data)
	case "resource_target":
		return db.UpsertResourceTarget(data)
	case "targets":
		return db.UpsertTargets(data)
	case "resource_users":
		return db.UpsertResourceUsers(data)
	case "whitelist":
		return db.UpsertWhitelist(data)
	case "add":
		return db.UpsertAdd(data)
	case "site_resource_clients":
		return db.UpsertSiteResourceClients(data)
	case "site_resource_roles":
		return db.UpsertSiteResourceRoles(data)
	case "site_resource_users":
		return db.UpsertSiteResourceUsers(data)
	case "2fa":
		return db.UpsertV2fa(data)
	case "add_role":
		return db.UpsertAddRole(data)
	case "remove_role":
		return db.UpsertRemoveRole(data)
	default:
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		...
		{Name: "health_checks", ParentTable: "orgs", ParentIDParam: "orgId", PathTemplate: "/org/{orgId}/health-checks", KeyField: ""},
		{Name: "idp_org", ParentTable: "idp", ParentIDParam: "idpId", PathTemplate: "/idp/{idpId}/org", KeyField: ""},
		{Name: "invitations", ParentTable: "orgs", ParentIDParam: "orgId", PathTemplate: "/org/{orgId}/invitations", KeyField: ""},
		{Name: "logs", ParentTable: "orgs", ParentIDParam: "orgId", PathTemplate: "/org/{orgId}/logs/connection", KeyField: ""},
		{Name: "org_access_tokens", ParentTable: "orgs", ParentIDParam: "orgId", PathTemplate: "/org/{orgId}/access-tokens", KeyField: ""},
		{Name: "org_clients", ParentTable: "orgs", ParentIDParam: "orgId", PathTemplate: "/org/{orgId}/clients", KeyField: ""},
		...
		{Name: "sites", ParentTable: "orgs", ParentIDParam: "orgId", PathTemplate: "/org/{orgId}/sites", KeyField: ""},
		{Name: "user_devices", ParentTable: "orgs", ParentIDParam: "orgId", PathTemplate: "/org/{orgId}/user-devices", KeyField: ""},
	}
}
