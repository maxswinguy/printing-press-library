...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/developer-tools/sec-edgar/internal/client"
	"github.com/mvanhorn/printing-press-library/library/developer-tools/sec-edgar/internal/config"
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "sec-edgar-pp-cli",
		Short: `Every SEC filing, every XBRL fact, every insider trade — synced into a local SQLite store you can pivot, search, and watch offline.`,
		Long: `Every SEC filing, every XBRL fact, every insider trade — synced into a local SQLite store you can pivot, search, and watch offline.
...
Health check: run 'sec-edgar-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage:  true,
		SilenceErrors: true,
		Version:       version,
	}
	rootCmd.SetVersionTemplate("sec-edgar-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'sec-edgar-pp-cli profile list')")
	...
	rootCmd.AddCommand(newSubmissionsPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())

	// Hand-authored novel commands (absorbed + transcendence). Each maps to a
	// row in the absorb manifest.
	rootCmd.AddCommand(newFilingsCmd(flags))
	rootCmd.AddCommand(newSecSearchCmd(flags))
	rootCmd.AddCommand(newFeedCmd(flags))
	rootCmd.AddCommand(newStatusCmd(flags))
	rootCmd.AddCommand(newSicCmd(flags))
	rootCmd.AddCommand(newWatchlistCmd(flags))
	rootCmd.AddCommand(newInsiderClusterCmd(flags))
	rootCmd.AddCommand(newWatchCmd(flags))
	rootCmd.AddCommand(newIndustryBenchCmd(flags))
	rootCmd.AddCommand(newCrossSectionCmd(flags))
